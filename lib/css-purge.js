...
const CPEVENTS = new CSSPurgeEmitter();
...
var OPTIONS = {
	file_output: CONFIG.options.file_output,
	input_files: CONFIG.options.input_files,
	input_html_files: CONFIG.options.input_html_files,
	input_js_files: CONFIG.options.input_js_files,
...
var reduceRulesListNameCount = 0;
...
function readReduceDeclarations(jsonConfigIn = '') {
	if (jsonConfigIn) {
		...
		readStream.on('data', function(chunk) {  
			...
			CPEVENTS.emit('CONFIG_READ_REDUCE_PROPS_ERROR');
...
function readConfig(configFileLocationIn = '', optionsIn = null) {
	...
	if (configFileLocationIn == '') {
		...
		if (configFileLocationIn == 'cmd_default') {
			...
			if (CONFIG.options.file_output) {
				OPTIONS.file_output = CONFIG.options.file_output;
			}
			if (CONFIG.options.verbose) {
				OPTIONS.verbose = CONFIG.options.verbose;
			}

			summary.files.output.push(CONFIG.options.file_output);
...
function processCSSFiles(filesIn = null, htmlFilesIn = null, optionsIn = null, configFileLocationIn = '') {
	
	if (!hasReadReduceDeclarations && optionsIn === null) {
		readReduceDeclarations();
	}

	CPEVENTS.on('CONFIG_READ_REDUCE_PROPS_END', function(){

		if (configFileLocationIn != 'cmd_default') {
			readConfig(configFileLocationIn);	
		}
		
		CPEVENTS.on('CONFIG_READ_END', function(){
			// console.log(CONFIG)

			//HTML Files
			var htmlFiles = CONFIG.options.input_html_files;

			if (htmlFilesIn !== null && htmlFilesIn !== undefined) {

				htmlFiles = htmlFilesIn;
			}


			//CSS Files
			var files = CONFIG.options.input_files;

			if (filesIn !== null && filesIn !== undefined) {
			...
			if (files !== '') {
				...
				CPEVENTS.on('CSS_READ_END', function(){
					...
					if (htmlFiles == '' || htmlFiles === undefined || OPTIONS.special_reduce_with_html == false) {
						processCSS();	
					}
			...
			if (htmlFiles !== '' && htmlFiles !== undefined && OPTIONS.special_reduce_with_html) {
	...
	}); //end of reduce config read
	
	if (!hasReadReduceDeclarations && optionsIn !== null && optionsIn['reduceConfig']) {
		if (optionsIn['reduceConfig'] === undefined || optionsIn['reduceConfig'] === null) {
		...
		try {
			...
			if (fileStats.isDirectory()) {
				...
				try {
					...
					console.log(error("Directory read error: Something went wrong while reading the directory, check your [input_html_files] in " + configFileLocation + " and please try again."));
					console.log(e);
					process.exit(1);
				}
			...
			console.log(error("CSS File read error: Something went wrong while reading the file(s), check your [input_html_files] in " + configFileLocation + " and please try again."));
			console.log(e);
			process.exit(1);
		}
...
function processCSS(cssDataIn = null, optionsIn = null) {
	...
	if (OPTIONS.verbose) { console.time(logoRed("Purged in")); }
	...
	//Detect via HTML
	if (OPTIONS.special_reduce_with_html) {
		...
		var htmlResults = processHTML(selectors);
		...
		try {
			if (OPTIONS.format_4095_rules_legacy_limit) {
				...
				if (noOutputFilesNeeded == 1) {
					...
					write(OPTIONS.file_output, outputCSS);
					filesizeKB = getFilesizeInKiloBytes(OPTIONS.file_output);
					...
					for (i = 0; i < rulesGroupsLen; i++) {
						...
						outputCSS = trimCSS(outputCSS);
						outputCSS = restoreHacks(outputCSS);
						outputFilename = OPTIONS.file_output.substr(0, OPTIONS.file_output.length-4) + '_' + i + '.css';
				...
				outputCSS = trimCSS(outputCSS);
				outputCSS = restoreHacks(outputCSS);
				if (OPTIONS.file_output === null || OPTIONS.file_output === undefined || OPTIONS.file_output == '') {
	...
	if (OPTIONS.verbose) {
		console.timeEnd(logoRed("Purged in"));
	...
	if (OPTIONS.file_output === null || OPTIONS.file_output === undefined || OPTIONS.file_output == '') {
		...
		console.log(OPTIONS.file_output)
		process.exit(1);
	}
}
