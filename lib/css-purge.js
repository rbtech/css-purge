...
function CSSPurge() {
    ...
    var OPTIONS = {
        ...
        css_raw: CONFIG.options.css_raw,
        html: CONFIG.options.html,
        html_raw: CONFIG.options.html_raw,
    ...
    function processHTML(cssSelectors = [], htmlDataIn = null, htmlOptionsIn = null) {
        ...
        } else if (typeof(OPTIONS.html_raw) !== 'undefined' && OPTIONS.html_raw !== '' && OPTIONS.special_reduce_with_html) {
            var htmlString = OPTIONS.html_raw.replace(/ /g, '');
            processHTMLSelectors(cssSelectors, htmlDataIn, htmlOptionsIn);
    ...
    function processRawCSS(optionsIn = null, configFileLocationIn) {
        if (optionsIn !== null && optionsIn !== undefined) {
            for (key in optionsIn) {
                OPTIONS[key] = optionsIn[key];
            }
        ...
        if (OPTIONS.verbose) {
            ...
            console.time(logoRed('Purged ' + timeLabel + ' in '));
            console.log(info('Process - CSS'));
        }

        if (OPTIONS.css_raw) {
            var cssData = OPTIONS.css_raw;
            var duplicates = [];
            ...
            try {
                var ast = css.parse(cssData);
                OPTIONS.verbose && console.log(info('Raw css parsed successfully'))
                ...
                OPTIONS.verbose && console.log(info('Process - Rules - Base'));
                ...
                } //end of for loop of rules process

                //create html with font-size
                fRules.unshift({
                    type: 'rule',
                    selectors: ['html'],
                    declarations: [{
                ...
                if (OPTIONS.special_reduce_with_html && (OPTIONS.html_raw !== undefined && OPTIONS.html_raw !== '')) {
                    OPTIONS.verbose && console.log(info('Process - Raw HTML reduce'));

                    var rulesIn = ast.stylesheet.rules;
                    var selectors = [], rulesCount2 = 0, rulesCount3 = 0, rulesCount4 = 0;
                    var ignoreFound = false;
                    var htmlDataIn = OPTIONS.html_raw;
                    ...
                    //remove duplicates
                    selectors = Array.from(new Set(selectors));
                    OPTIONS.verbose && console.log(info('Process - Raw HTML reduce - processHTMLSelectors'));
                    processHTMLSelectors(selectors, htmlDataIn);
                    summary.selectors_removed = selectors;
                    outputCSS = processHTMLResults(rulesIn, selectors);
                }
            ...
            console.error(error('CSS raw content not provided in css_raw option'));
        }
    }
    ...
    this.purgeRawCSS = function(optionsIn) {
        return processRawCSS(optionsIn)
    }
} //end of CSSPurge
...
var purgeTriage = {
    ...
    purgeRawCSS: function(optionsIn) {
        var cssPurge = new CSSPurge();
        purgeSessions.push(cssPurge);
        return cssPurge.purgeRawCSS(optionsIn);
    }
}
