...
function CSSPurge() {
	...
	function restoreHacks(outputCSSIn) {
		...
		//tokens - ignore + trim keyframes
		outputCSSIn = outputCSSIn.replace(/(._7token_)[0-9]*\{color\:\#fff\}/g, function(match){
			tmpVal = token7_vals[match.substring(9,match.length-12)-1];
			...
			if (rules[i] !== undefined && rules[i].declarations !== undefined  && rules[i].type == 'rule') {
				...
				if (OPTIONS.shorten_border || OPTIONS.shorten) {
					...
					if (
						...
						if (
							borderHasInherit == false
							&& (borderProps.indexOf('border-color') != -1 && borderValues[borderProps.indexOf('border-color')].split(' ').length == 1) //multi-color (border around squares, etc.) check - only do if single
	...
	function processRules(rules) {
		...
		if (rules !== undefined) {
			...
			if (OPTIONS.new_reduce_common_into_parent) {
				...
				var hierachyKeys = [];
				var hierachyLen = 0;
				var selectedHierachyLevel = 0;
				var commonParentsIdx = -1;
				var commonParentsKeys = [];
				var commonParents = [];
				var commonParentsLen = 0;
				var commonParent = '';
				var commonParent2 = '';
				var commonParentTotal = 0;
				var commonParentDeclarations = [];
				var commonLineage = '';
				var newParentDeclarations = [];
				var processedCommonParentsChildren = [];
				var classLineage = '';
				var parentClassLineage = '';
				var lineageLabel = '';
				var tmpDeclarations = [];
				...
				for (i = 0; i < rCount; ++i) {
					...
					if (rules[i].selectors !== undefined) {
						for (j = 0; j < rules[i].selectors.length; j++) { //each comma delimited
							
							if (rules[i].selectors[j].indexOf('.') != -1) {
								...
								for (k = 0; k < classLineage.length; k++) { //depth of hierachy
				...
				// get declarations
				for (i = 0; i < commonParentsKeys.length; i++) {

					if (rules[commonParentsKeys[i].index].declarations !== undefined) {

						for (j = 0; j < rules[commonParentsKeys[i].index].declarations.length; j++) {
					...
					Object.keys(newParentDeclarations).forEach(function(key, index) {
						...
						if (commonParentsKeys[i] !== undefined && commonParentsKeys[i].selector == key) {
						  ...
						  if (rules[commonParentsKeys[i].index] !== undefined && rules[commonParentsKeys[i].index].declarations !== undefined) {
						  	...
						  	dCount = tmpDeclarations.length;
							  
						  	for (j = 0; j < this[key].declarations.length; j++) { //each parent declaration

							  	//remove declarations
							  	for (k = 0; k < dCount; k++) { //each child declaration

							  		if (
							  			this[key].declarations[j] !== undefined
				...
				Object.keys(newParentDeclarations).forEach(function(key, index) {
				  ...
				  rules.splice(this[key].selectorIndex-1, 0, {
						type : 'rule',
						selectors: [key],
				...
				for (j = i+1; j < rCount; ++j) {
					...
					if (rules[i] !== undefined 
						...
						if (rules[i].selectors.toString() == rules[j].selectors.toString()) {
							...
							if ( (j-i) > 1 ) { //check comparison distance
			      		...
			      		amountRemoved = 1; //shift the ids by the amount removed
								for (l = 0; l < duplicate_ids.length-1; ++l) { // -1 to leave last behind
									...
									if (duplicate_ids[l+1] !== undefined) {
										...
										for (key2 in declarationsValueCounts) {
							      		...
							      		for (l = 0; l < duplicate_ids.length; ++l) {
										...
										if (rules[i].declarations[l].type == 'declaration') {
											...
											if (
												...
												if (rules[i].declarations[l].property == reduceRulesListName[k]) { 
													...
													// console.log(reduceRulesListName[k])
	...
	function processCSS(cssDataIn = null, optionsIn = null, callback = function(error, result){}) {
		...
		var continueCSSProcess = function(){
			...
			//tokens - ignore keyframes
			cssData = cssData.replace(/(@(-?)[a-zA-Z\-]*(keyframes)[a-zA-Z\-]*\s[a-zA-Z\-]*( ?,?))+{((\s*)(.*)(\r*)(\t*)(\n*))([0-9\%]*(\s*)(.*)(\r*)(\t*)(\n*))((\s*)(.*)(\r*)(\t*)(\n*))}/g, function(match){
				token7_vals.push(match.substr(0,match.length));
				return '._7token_'+token7_vals.length+'{color: #fff}';
			});

			//tokens - data:image
			cssData = cssData.replace(/url\(\"data:image\/([a-zA-Z]*);base64,([^\"]*)\"\)/g, function(match){
				token6_vals.push(match.substr(0,match.length));
				return '_6token_dataimage_'+token6_vals.length+':';
			});
